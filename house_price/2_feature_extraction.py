...
def main():
  ...
  fill_mean_features = ["GarageYrBlt"]
  string_features = [
...
def fill_nan_value(df, fill_most_features, fill_mean_features,
  ...
  """
  print("\n[Debug] Fill the NaN data of the dataset: ")
  feature_name = "Age"
  mean = df[feature_name].mean()
  print("For feature {}, fill with the average value: {}".format(feature_name, mean))
  df.loc[df[feature_name].isnull(), feature_name] = mean
  preview_examples_of_features(df, [feature_name])

  # Example: Series({"S": 644, "C": 168, "Q": 77})
  feature_name = "Embarked"
  value_count_series = df[feature_name].value_counts()
  most_value = value_count_series.index[0]
  print("For feature {}, fill with the most value: {}".format(feature_name, most_value))
  df[feature_name].fillna(most_value, inplace=True)
  preview_examples_of_features(df, [feature_name])
  """
...
def drop_string_features(df, feature_name_array):
  ...
  useless_string_features = ["Name", "Ticket"]
  print("Drop the useless string features: {}".format(useless_string_features))
  df.drop(useless_string_features, axis=1, inplace=True)

  missing_value_features = ["Cabin"]
  print("Drop the missing values features: {}".format(missing_value_features))
  df.drop(missing_value_features, axis=1, inplace=True)

  useless_converted_features = ["Age", "Fare", "Sex", "Embarked"]
  print("Drop the useless converted features: {}".format(useless_converted_features))
  df.drop(useless_converted_features, axis=1, inplace=True)
  """
